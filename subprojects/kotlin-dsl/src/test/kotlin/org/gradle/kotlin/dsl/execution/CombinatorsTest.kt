...
class CombinatorsTest {
    ...
    fun `can parse boolean literals`() {
        val parser = booleanLiteral

        assertSuccess(parser("true"))
        assertSuccess(parser("false"))
    ...
    fun `fails to parse no symbol as one or more`() {
        val parser = oneOrMore(symbol("foo"))
        assertFailure(
            parser(""),
